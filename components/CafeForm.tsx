...
export default function CafeForm({
    ...
    if (cafeId) {
      ...
      fetch(`/api/cafes/${cafeId}`)
        ...
        .then((data) => {
          ...
          // Clear photo gallery when loading existing cafe
          setAvailablePhotos([]);
        })
        .catch((err) => {
      ...
      // Clear photo gallery when creating new cafe
      setAvailablePhotos([]);
    }
  }, [cafeId, reset]);
  ...
  useEffect(() => {
    if (cafeAddresses) {
      setAddressesText(JSON.stringify(cafeAddresses, null, 2));
    }
  }, [cafeAddresses]);

  const handleFetchPhoto = async () => {
    if (!cafeName) {
      setError("Vyplň nejdřív název kavárny");
      return;
    }
    ...
    try {
      const params = new URLSearchParams({
        name: cafeName,
        ...(cafeLocation && { location: cafeLocation }),
      });
